...
Clean-separation test for r: the descriptor is about the ITEM's position in its decision lifecycle; it could be true of a doc our tooling never touched (→ not an M descriptor); it is recomputed from the doc's Q profile and never scored independently (→ not a second quality score). sol-s0..s2 / prd-r1..r3 stay NAMED but UNDEFINED (lazy fractal) — reserved vocabulary in contracts only. **(SUPERSEDED 2026-07-07: solution-doc lvl1 collapsed these into ONE `sol-r0..sol-r3` readiness ladder — PRD = the mature Solution Doc, one artifact; a second family would be a shadow axis violating this addendum's own single-projection rule. See `components/solution-doc/SPEC.md` §Readiness.)**
