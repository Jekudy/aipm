...
- **Use a real domain artifact as an EVAL fixture + format orientation ONLY**, never a gold standard — and say so in the SPEC. The real Avito PRD «[скрыто]» calibrated S7 without one uneven doc setting the bar (that IS the false-ABSENT test). (SPEC.md:44; worklog:180-182) `[all-modules]`
