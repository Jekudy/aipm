...
invariant≠postcondition). Domain calibration from the real Avito PRD «[скрыто]»
(Confluence [скрыто]) — used as an EVAL example (per user: not a gold standard), saved as a
...
needs ≥3 real PRDs with false-ABSENT rate (1/3: «[скрыто]»).
...
## 2026-07-16 · Stage 4 Phase A: solution-backlog lvl1 SPEC reverse-ratified (M0→M1 draft, gate pending)

- **Reverse-ratification done** (per `docs/handoffs/stage-4-solution-backlog-lvl1.md` §2A): the entry shape derived from what the TWO live writers actually put on disk (opp-doc promotion: Markdown `## sol-A` + subsections; sol-doc write-back: YAML block with `candidate_id`/`status`/`readiness`/`decision`) + canon. Confirmed the doubled two-serializations defect AND sol-doc internal drift (W2-1 vs case-9-p3: `acceptance_criteria` vs `_seed` key, list-vs-scalar invariants, decision/guardrails/key_assumptions presence). **Ratified ONE canonical serialization: the sol-doc YAML-block form extended to the full canon folded set** — one fenced block per entry, 18 mandatory keys, quoted free-text scalars, ISO dates, `sol_state` retired, lifecycle four seeded at promotion (`status: draft`, `decision: —`, `readiness: sol-r0 (promoted <date>)`, `updated_at`). Forward-compatible with the lvl2 table/Sheets-via-gws target (keys = columns).
- **Deliverables:** `components/solution-backlog/SPEC.md` (lvl1 draft replacing the M0 stub; mirror of the opp-backlog SPEC shape) + `docs/handoffs/stage-4-phase-a-gate.md` (drift register D1–D15, spec-friction F1–F8 for the orchestrator, ranking decision, trap-checklist §4 resolutions, open questions).
- **Ranking (§2C) decided:** PM-set ordinal `rank` (optional) + mandatory verbatim `pm_priority` passthrough (MoSCoW/RICE never converted/aggregated — real-wave R2); no computed score (EARS invariant); surfacing/shortlists deferred. C1/C3 ratified: already-decided rows honestly admissible (status/decision elicited); parent = hard blocker WITH first-class route; parent EXISTENCE accepted on assertion in lvl1 (no cross-file read).
- **Cross-consistency sweep `wf_f97b9845-177`** (7 Opus lenses: both writers, canon, sister-mirror, trap-checklist, red-team, inherited-fixes): 3 major findings, all fixed pre-gate — YAML quoting discipline (bare Russian `: ` broke parse — proven by parser test), parent-existence decision made explicit, admission guard de-jargoned (sister C7). Canonical entry validated by a runnable PyYAML check (poison chars parse; unquoted form fails as expected).
- **STOP:** Phase B (skill build) starts ONLY after explicit user approval at the visual gate; SPEC freezes on approval.

---

## 2026-07-16 · Stage 4 Phase B DONE: solution-backlog lvl1 built to M2 (plugin v0.5.1) + real-data волна (fallback)

- **Gate approved 2026-07-16** → SPEC заморожен (M0→M1). **Build workflow `wf_59c2356e-448`** (37 агентов; Fable = план/chief-gate, Opus = судьи/аудиты/фиксеры/регистрация, Sonnet = симуляторы): plan (AC-coverage PASS с 1-го раунда) → 3 deliverables built+verified (0 fix-раундов) → **smoke 5/5 → stress 6/6** (P6 bulk-миграция перегнан после API-обрыва симулятора: 2 записи из 4 строк, ноль-райт не повторился) → **интеграционная проверка обоих живых писателей PASS** (весь дрейф = уже записанные F1–F8; уточнение: `sol_state` — артефакт фикстуры stage-2, не поведение opp-doc) → 3 аудита PASS → регистрация 0.4.1→0.5.0 (обе манифесты, версия прочитана живьём) → **chief-gate SATISFIED** (20/20 замороженных хэшей, git aipm чистый, write-ops доказаны на байтах).
- **Post-wave патч S2-класса** (судья P6 поймал синтез key_assumptions/invariants на bulk-create): elicit-or-gap-marker «— не подтверждено PM», intent на русском PM, дубликат не просачивается в поля. Верифицирован `wf_1e7aa3f5-d5b`: адверсарное ревью PASS + ретесты 2/2 (RT2 — гейты под комбинированным давлением не смягчились, ноль мутаций). Зарегистрирован **0.5.1**, кэш байт-идентичен.
- **Real-data волна `wf_8d429639-304`** (19 агентов, fallback-режим): **находка №0 — вкладка «решения» реального шита ПУСТА** (0 строк, 0 заголовков; сестринская «возможности» ~118 строк) — проверено вживую read-only, зафиксировано без диагноза причины; волна на кандидатах, произведённых самим плагином. T1 handshake промоушена (drift-форма opp-doc → gated-конверсия), T2 честность пустой миграции, T3 смешанные формы + ранжирование, T4 голосовой intake. **4/4 spec-pass, pm_scores 6/7/7/7** (персистированы поштучно — урок stage-2). Корзины после челленджа: 6 [skill-fix] (не применены) / 4 [spec-friction] / 12 [lvl2-req]; 10 находок убито как ломающие контракт. REPORT: `pm-flow/.fixtures/stage-4-real-wave/REPORT.md` (русский, пересказ, без идентификаторов).
