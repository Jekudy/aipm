...
- **Never auto-default a field (S2); no silent lifecycle lie (S3); conversational/batch provenance counts (S4).** Marked-verbatim strings are reproduced byte-exactly (sentence-initial capitalization at the start of an utterance is allowed); improvised Russian carries no jargon (no "S-1"/"lvl1"/section codes at the PM). *(Scope clarification 2026-07-16, wave-driven:* the no-jargon rule targets refusal/admission/guard strings and free prose — its C7 origin; schema FIELD NAMES are permitted inside an S-1 change inventory, where per-field precision is mandatory and cannot be paraphrased.)
