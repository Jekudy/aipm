...
| INF-DISC-01 | D2 inferential essentials | Для claim задан наблюдаемый результат, при котором claim не поддерживается. |
...
| R-REGISTRY-01 | Если `registry_configured=true` | Зафиксированы version, query/filter, дата и matches/0. |
...
Canonical candidate вычисляется первым сработавшим правилом и затем нормализуется в exposed contract:

1. Невозможно определить transition/track/thread или обязательный источник → canonical `NO_CANDIDATE`, exposed `BLOCKED`, `d5_mode=null`.
2. Для D5: `ST-00=PARK` → canonical `D5_PARK_CANDIDATE`, exposed `STOP_CANDIDATE`, `d5_mode=PARK`; это сохраняется даже если `DB-04`, `ST-02` или `ST-03P` дают `NOT_READY`.
3. Для D5: `ST-00=STOP` и `ST-01` имеет закрытую причину → canonical `D5_STOP_CANDIDATE`, exposed `STOP_CANDIDATE`, `d5_mode=STOP`; это сохраняется даже если `DB-04` или `ST-02` дают `NOT_READY`.
4. Для D2: essential occurrence или harm=`REFUTES` → canonical `D5_STOP_CANDIDATE`, exposed `STOP_CANDIDATE`, `d5_mode=STOP`.
5. Для D2: любой essential не `SUPPORTS`, relation не готова или dossier не ready → canonical/exposed `CONTINUE_CANDIDATE`.
6. Для D2: ready dossier и каждый active essential=`SUPPORTS` → canonical/exposed `D2_CANDIDATE`, `d5_mode=null`.
7. Для D1: canonical/exposed `CONTINUE_CANDIDATE`; готовность к взятию сигнала показывает `dossier_status`, а не отдельный candidate.
