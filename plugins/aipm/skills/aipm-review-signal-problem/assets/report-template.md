...
- При `BLOCKED` по входу после «Вердикта» идёт ровно один вопрос с вариантами словами; блоки «Чего не хватает», «Что перевернуло бы вывод» и «Границы» опускаются, машинный слой остаётся. При `BLOCKED_SOURCE` вердикт «Проверить нельзя без …», остальные блоки остаются.
- Заголовок всегда с названием документа: «## Проверка: {название}».
...
Правила: та же дисциплина цитат; критерий называется словами, без ID и кодов; пройденные проверки показываются полностью, скрытых нет; счётчики здесь допустимы, проценты и баллы нет. Проверки ➖ с одной и той же причиной (например, «нет блока воздействий») сворачиваются в одну строку с перечнем названий: по строке на каждую не нужно.
