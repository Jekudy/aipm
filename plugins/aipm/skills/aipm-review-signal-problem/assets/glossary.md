...
| BLOCKED по входу (INVALID_INPUT, TRACK_NOT_SUPPORTED, CENTRAL_CLAIM_UNRESOLVED, DB-09) | Проверка не началась: {причина словами} |
| BLOCKED по источникам (BLOCKED_SOURCE) | Проверить нельзя без {каких источников}: {причина словами}; препятствия и границы показываются как обычно |
...
| ABSENT | в документе этого нет (пишется в строке «Где», не в заголовке пункта) |
| INVALID_FORM | есть, но не в проверяемом виде (пишется в строке «Где», не в заголовке пункта) |
...
| CANNOT_ASSESS | В документе: данных для этой проверки нет |
